...
from src.models import train_lgb_fold
...
def objective(trial, X_tr_0, y_tr_0, X_va_0, y_va_0, X_tr_1, y_tr_1, X_va_1, y_va_1):
    ...
    # Evaluate on Fold 0
    _, val_pred_0, _, _ = train_lgb_fold(X_tr_0, y_tr_0, X_va_0, y_va_0, None, 0, params=params)
    auc_0 = roc_auc_score(y_va_0, val_pred_0)
    
    # Prune early if fold 0 is poor
    if auc_0 < 0.9395:
        return auc_0
        
    # Evaluate on Fold 1
    _, val_pred_1, _, _ = train_lgb_fold(X_tr_1, y_tr_1, X_va_1, y_va_1, None, 1, params=params)
    auc_1 = roc_auc_score(y_va_1, val_pred_1)
    
    return (auc_0 + auc_1) / 2.0

def main():
    print("Loading data for LightGBM tuning...")
    ...
    print("Starting Optuna tuning for LightGBM (25 trials)...")
    start_t = time.time()
    ...
    with open(os.path.join(OUTPUT_DIR, "best_params_lgb.json"), "w") as f:
        json.dump(study.best_params, f, indent=2)
        
    print(f"Saved best params to {OUTPUT_DIR}/best_params_lgb.json")
